...
class SemanticKITTI(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_2_label = np.zeros((1, height, width), dtype=np.uint8) 
        if self.imageset == 'test':
            annotated_data = np.expand_dims(np.zeros_like(raw_data[:, 0], dtype=int), axis=1)
            instance_label = np.expand_dims(np.zeros_like(raw_data[:, 0], dtype=int), axis=1)
            weak_label = np.expand_dims(np.zeros_like(raw_data[:, 0], dtype=int), axis=1)
            image_2_label = np.zeros((height, width, 3), dtype=np.uint8) 
